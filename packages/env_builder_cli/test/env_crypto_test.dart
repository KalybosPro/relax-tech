  ...
  group('EnvCrypto', () {
    group('encryptFile', () {
      test('should encrypt a valid env file', () async {
        ...
        final inputFile = File(p.join(tempDir.path, '.env'));
        await inputFile.writeAsString(testContent);
        final outputFile = File(p.join(tempDir.path, '.env.encrypted'));
      ...
      test('should create valid JSON format for encrypted file', () async {
        ...
        expect(
          parsed,
          contains('version'),
          reason: 'Should contain version field',
        );
      });
    ...
    group('decryptFile', () {
      test('should decrypt a previously encrypted file', () async {
        ...
        final encryptedFile = File(p.join(tempDir.path, '.env.encrypted'));
        final decryptedFile = File(p.join(tempDir.path, '.env.decrypted'));
        const password = 'testPassword123';
      ...
      test('should fail with wrong password', () async {
        ...
        await testFile.writeAsString(originalContent);
        final encryptedFile = File(p.join(tempDir.path, '.env.encrypted'));
        final wrongDecryptedFile = File(
          p.join(tempDir.path, '.env.wrongdecrypt'),
        );
        ...
        expect(
          true,
          true,
          reason: 'Should handle wrong password without throwing exception',
        );
      });
      ...
      test('should handle non-existent encrypted file gracefully', () async {
        ...
        final outputFile = File(p.join(tempDir.path, '.env.decrypted'));
        const password = 'testPassword123';
      ...
      test('should handle invalid JSON format gracefully', () async {
        // Arrange
        final invalidFile = File(p.join(tempDir.path, '.env.invalid'));
...
DEBUG_MODE=false
LOG_LEVEL=info''';
        final inputFile = File(p.join(tempDir.path, '.env.prod'));
        await inputFile.writeAsString(testContent);
        final outputFile = File(p.join(tempDir.path, '.env.prod.encrypted'));
      ...
      test('should encrypt file with special characters', () async {
        ...
        final inputFile = File(p.join(tempDir.path, '.env.special'));
        await inputFile.writeAsString(testContent);
        final outputFile = File(p.join(tempDir.path, '.env.special.encrypted'));
        const password = 'p@ssw0rd_with_spec1al_ch@rs!';

        // Act
        await EnvCrypto.encryptFile(inputFile.path, outputFile.path, password);
        final decryptedFile = File(
          p.join(tempDir.path, '.env.special.decrypted'),
        );
        await EnvCrypto.decryptFile(
          outputFile.path,
          decryptedFile.path,
      ...
      test('should encrypt file with long content', () async {
        ...
        final inputFile = File(p.join(tempDir.path, '.env.long'));
        await inputFile.writeAsString(testContent);
        final outputFile = File(p.join(tempDir.path, '.env.long.encrypted'));
        const password = 'testPassword123';

        // Act
        await EnvCrypto.encryptFile(inputFile.path, outputFile.path, password);
        final decryptedFile = File(p.join(tempDir.path, '.env.long.decrypted'));
        await EnvCrypto.decryptFile(
          outputFile.path,
          decryptedFile.path,
          password,
    ...
    group('password variations', () {
      test('should use same key for same password', () async {
        ...
        final encFile1 = File(p.join(tempDir.path, '.env.enc1'));
        final encFile2 = File(p.join(tempDir.path, '.env.enc2'));
      ...
      test('should handle passwords with unicode characters', () async {
        // Arrange
        const testContent = 'API_KEY=secret123';
        final inputFile = File(p.join(tempDir.path, '.env.unicode'));
        await inputFile.writeAsString(testContent);
        final outputFile = File(p.join(tempDir.path, '.env.unicode.encrypted'));
        ...
        final decryptedFile = File(
          p.join(tempDir.path, '.env.unicode.decrypted'),
        );
        await EnvCrypto.decryptFile(
          outputFile.path,
          decryptedFile.path,
